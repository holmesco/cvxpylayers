...
class TestCvxpyLayer(unittest.TestCase):
    ...
    def test_least_squares(self):
        ...
        def lstsq(A, b):
            return torch.solve(
                (A_th.t() @ b_th).unsqueeze(1), A_th.t() @ A_th + torch.eye(n).double()
            )[0]
        ...
        self.assertAlmostEqual(torch.norm(grad_A_cvxpy - grad_A_lstsq).item(), 0.0)
        self.assertAlmostEqual(torch.norm(grad_b_cvxpy - grad_b_lstsq).item(), 0.0)
    ...
    def test_logistic_regression(self):
        ...
        torch.autograd.gradcheck(
            layer_eps, (X_tch, lam_tch), eps=1e-4, atol=1e-3, rtol=1e-3
        )
    ...
    def test_entropy_maximization(self):
        ...
        A_tch, b_tch, F_tch, g_tch = map(
            lambda x: torch.from_numpy(x).requires_grad_(True), [A_np, b_np, F_np, g_np]
        )
        torch.autograd.gradcheck(
            lambda *x: layer(*x, solver_args={"eps": 1e-12, "max_iters": 10000}),
    ...
    def test_lml(self):
        ...
        x_th = torch.DoubleTensor([1.0, -1.0, -1.0, -1.0])
    ...
    def test_sdp(self):
        ...
        A_tch = [torch.randn(n, n, requires_grad=True).double() for _ in range(p)]
        b_tch = [torch.randn(1, 1, requires_grad=True).double() for _ in range(p)]
        ...
        layer = CvxpyLayer(prob, [C] + A + b, [X], constraints=[constraints[0]])
        torch.autograd.gradcheck(
    ...
    def test_broadcasting(self):
        ...
        def lstsq(A, b):
            return torch.solve(
                (A.t() @ b).unsqueeze(1), A.t() @ A + torch.eye(n).double()
            )[0]
        ...
        self.assertAlmostEqual(
            torch.norm(grad_A_cvxpy / n_batch - grad_A_lstsq).item(), 0.0
        )
        self.assertAlmostEqual(torch.norm(grad_b_cvxpy[0] - grad_b_lstsq).item(), 0.0)
    ...
    def test_shared_parameter(self):
        ...
        A_tch = torch.randn(m, n, requires_grad=True)
        solver_args = {"eps": 1e-10, "acceleration_lookback": 0, "max_iters": 10000}

        torch.autograd.gradcheck(
            lambda A: torch.cat(
                [
                    layer1(A, solver_args=solver_args)[0],
                    layer2(A, solver_args=solver_args)[0],
                ]
            ),
            (A_tch,),
        )
    ...
    def test_equality(self):
        ...
        torch.autograd.gradcheck(
            lambda b: layer(b, solver_args={"eps": 1e-10, "acceleration_lookback": 0})[
                0
            ].sum(),
            (b_tch,),
        )
    ...
    def test_basic_gp(self):
        ...
        torch.autograd.gradcheck(
            lambda a, b, c: layer(
                a, b, c, solver_args={"eps": 1e-12, "acceleration_lookback": 0}
            )[0].sum(),
            (a_tch, b_tch, c_tch),
            atol=1e-3,
            rtol=1e-3,
