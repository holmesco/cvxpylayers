...
class CvxpyLayer(torch.nn.Module):
    ...
    def __init__(self, problem, parameters, variables, constraints=[], gp=False):
        ...
        Args:
          ...
          constraints: A list of CVXPY Constraints in the problem; the order of the
                     Constraints determines the order of the optimal dual variable
                     values returned from the forward pass. Dual variables are returned
                     after primal variables in the list.
          gp: Whether to parse the problem using DGP (True or False).
          use_dcone: Whether to retrieve the batch dual solutions.
        """
        ...
        if not set(constraints).issubset(set(problem.constraints)):
            raise ValueError(
                "Argument constriants must be a subset of " "problem.constraints"
            )
        ...
        if not isinstance(constraints, list) and not isinstance(variables, tuple):
            raise ValueError(
                "The layer's constraints must be provided as " "a list or tuple"
            )
...
def split_dual(sltn, constraint, constr_map):
    ...
    """
    offset = constr_map[constraint.id]
    if isinstance(constraint, PSD):
        dim = constraint.shape[0]
...
def _CvxpyLayerFn(
    ...
    class _CvxpyLayerFnFn(torch.autograd.Function):
        ...
        def forward(ctx, *params):
            ...
            sol = [[] for _ in range(len(variables) + len(constraints))]
            for i in range(ctx.batch_size):
                # Add primal variable solutions
                ...
                # Add dual variable solutions
                for j, c in enumerate(constraints):
                    dual_val = split_dual(ys[i], constraint=c, constr_map=constr_map)
                    sol[j + len(variables)].append(
                        to_torch(dual_val, ctx.dtype, ctx.device).unsqueeze(0)
                    )
